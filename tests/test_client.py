#!/usr/bin/env python3
"""Tests for client mode implementation."""
...
async def test_connect_to_server_success() -> None:
    ...
    from pclipsync.client import connect_to_server
...
async def test_run_client_with_retry_clears_hash_state(mock_state: MagicMock) -> None:
    ...
    from pclipsync.client import run_client_with_retry
    ...
    with patch("pclipsync.client.connect_to_server", new_callable=AsyncMock) as mock_conn:
        mock_conn.return_value = (mock_reader, mock_writer)
        with patch("pclipsync.client.run_sync_loop", new_callable=AsyncMock) as mock_sync:
...
async def test_run_client_with_retry_retries_on_connection_error(
    ...
    with patch("pclipsync.client.connect_to_server", side_effect=track_connection):
        with patch("pclipsync.client.run_sync_loop", new_callable=AsyncMock) as mock_sync:
            mock_sync.side_effect = KeyboardInterrupt()
